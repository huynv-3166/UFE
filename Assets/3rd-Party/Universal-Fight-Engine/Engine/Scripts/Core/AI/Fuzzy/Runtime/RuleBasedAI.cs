...
public class RuleBasedAI : RandomAI{
	...
	protected virtual void AddValidMoves(
		...
		float maxGaugePoints = (float)(self.myInfo.maxEPS);
			...
			for (int j = 0; j < move.attackMoves.Length; ++j){
				...
				if (moveInfo != null && self.MoveSet.ValidateMoveExecution(moveInfo)){
					...
					if (moveInfo.hits.Length > 0){
						if (ai.advancedOptions.reactionParameters.enableDamageFilter){
							damage /= (float)(opponent.myInfo.healthPoints);
	...
	protected virtual void RequestAIUpdate(ControlsScript self, ControlsScript opponent, float deltaTime){
		if (self != null && opponent != null){
			...
			float characterGaugeSelf = (float)self.currentGaugesPoints[0] / (float)(self.myInfo.maxEPS);
            float characterHealthSelf = (float)self.currentLifePoints / (float)(self.myInfo.healthPoints);
			...
			if (myMoveInfo != null){
				...
				attackGaugeSelf = myMoveInfo.gauges.Length > 0 ? (float)(myMoveInfo.gauges[0]._gaugeUsage / self.myInfo.maxEPS) : 0;
            ...
            float characterGaugeOpponent = (float)opponent.currentGaugesPoints[0] / (float)(opponent.myInfo.maxEPS);
            float characterHealthOpponent = (float)opponent.currentLifePoints / (float)(opponent.myInfo.healthPoints);
			...
			if (opMoveInfo != null){
				...
				attackGaugeOpponent = opMoveInfo.gauges.Length > 0 ? (float)(opMoveInfo.gauges[0]._gaugeUsage / opponent.myInfo.maxEPS) : 0;
