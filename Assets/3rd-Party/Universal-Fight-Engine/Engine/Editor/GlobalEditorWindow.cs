...
public class GlobalEditorWindow : EditorWindow
{
    ...
    private bool basicgaugeOptions;
    ...
    void Populate()
    {
        ...
        if (selection.Length > 0)
        {
            ...
            UFE.currentEditorConfig = globalInfo;
        }
    ...
    public void OnGUI()
    {
        ...
        scrollPos = EditorGUILayout.BeginScrollView(scrollPos);
        {
                ...
                if (deploymentOptions)
                {
                    EditorGUILayout.BeginVertical(subGroupStyle);
                    {
                        ...
                        if (globalInfo.deploymentOptions.deploymentType != DeploymentType.FullInterface)
                        {
                            ...
                            {*/
                            globalInfo.deploymentOptions.activeCharacters[0] = (TOHDragonFight3D.CharacterInfo)EditorGUILayout.ObjectField("Player 1 Character:", globalInfo.deploymentOptions.activeCharacters[0], typeof(TOHDragonFight3D.CharacterInfo), false);
                            ...
                            EditorGUILayout.Space();
                            globalInfo.deploymentOptions.activeCharacters[1] = (TOHDragonFight3D.CharacterInfo)EditorGUILayout.ObjectField("Player 2 Character:", globalInfo.deploymentOptions.activeCharacters[1], typeof(TOHDragonFight3D.CharacterInfo), false);
                ...
                if (cameraOptions)
                {
                    EditorGUILayout.BeginVertical(subGroupStyle);
                    {
                        ...
                        if (globalInfo.gameplayType == GameplayType._2DFighter)
                        {
                            ...
                            globalInfo.cameraOptions.calculateInitialDistance = EditorGUILayout.Toggle("Re-Calculate Initial Distance:", globalInfo.cameraOptions.calculateInitialDistance);
                        }
#if !UFE_LITE && !UFE_BASIC
            ...
            EditorGUILayout.BeginVertical(rootGroupStyle);
            {
                EditorGUILayout.BeginHorizontal();
                {
                    basicgaugeOptions = EditorGUILayout.Foldout(basicgaugeOptions, "Basic Attack Gauges (" + globalInfo.basicgauges.Length + ")", foldStyle);
                    helpButton("move:gauge");
                }
                EditorGUILayout.EndHorizontal();

                if (basicgaugeOptions)
                {
                    EditorGUILayout.BeginVertical(subGroupStyle);
                    {
                        EditorGUI.indentLevel += 1;
                        List<int> castingValues = new List<int>();
                        foreach (GaugeInfo gaugeInfo in globalInfo.basicgauges)
                        ...
                        for (int i = 0; i < globalInfo.basicgauges.Length; i++)
                        {
                            EditorGUILayout.Space();
                            EditorGUILayout.BeginVertical(arrayElementStyle);
                            {
                                ...
                                EditorGUILayout.BeginHorizontal();
                                {
                                    globalInfo.basicgauges[i].targetGauge = (GaugeId)EditorGUILayout.EnumPopup("Target Gauge:", globalInfo.basicgauges[i].targetGauge, enumStyle);
                                    if (GUILayout.Button("", "PaneOptions"))
                                    {
                                        PaneOptions<GaugeInfo>(globalInfo.basicgauges, globalInfo.basicgauges[i], delegate (GaugeInfo[] newElement) { globalInfo.basicgauges = newElement; });
                                    }
                                }
                                EditorGUILayout.EndHorizontal();
                                EditorGUI.indentLevel += 1;
                                ...
                                globalInfo.basicgauges[i].castingFrame = EditorGUILayout.IntSlider("Frame:", globalInfo.basicgauges[i].castingFrame, 0, maxCastingFrame);
                                globalInfo.basicgauges[i]._gaugeGainOnMiss = StyledSlider("Gauge Gain (%)", (float)globalInfo.basicgauges[i]._gaugeGainOnMiss, EditorGUI.indentLevel, 0, 100);

                                EditorGUILayout.Space();
                                EditorGUIUtility.labelWidth = 200;
                                EditorGUI.BeginDisabledGroup(globalInfo.basicgauges[i].stopDrainingGauge);
                                {

                                    globalInfo.basicgauges[i].startDrainingGauge = EditorGUILayout.Toggle("Start Draining", globalInfo.basicgauges[i].startDrainingGauge, toggleStyle);
                                    if (globalInfo.basicgauges[i].startDrainingGauge)
                                    {
                                        globalInfo.basicgauges[i].inhibitGainWhileDraining = EditorGUILayout.Toggle("- Inhibit Gauge Gain", globalInfo.basicgauges[i].inhibitGainWhileDraining, toggleStyle);
                                        globalInfo.basicgauges[i]._gaugeDPS = EditorGUILayout.FloatField("- Drain Per Second (%):", (float)globalInfo.basicgauges[i]._gaugeDPS);
                                        globalInfo.basicgauges[i]._totalDrain = EditorGUILayout.FloatField("- Total Drain (%):", (float)globalInfo.basicgauges[i]._totalDrain);
                                        globalInfo.basicgauges[i].DCMove = (MoveInfo)EditorGUILayout.ObjectField("- Move (Drain Complete):", globalInfo.basicgauges[i].DCMove, typeof(MoveInfo), false);
                                        globalInfo.basicgauges[i].DCStance = (CombatStances)EditorGUILayout.EnumPopup("- Stance (Drain Complete):", globalInfo.basicgauges[i].DCStance, enumStyle);
                                    }
                                }
                                EditorGUI.EndDisabledGroup();
                                globalInfo.basicgauges[i].stopDrainingGauge = EditorGUILayout.Toggle("Stop Draining", globalInfo.basicgauges[i].stopDrainingGauge, toggleStyle);
                                EditorGUIUtility.labelWidth = 150;


                                EditorGUILayout.Space();
                                SubGroupTitle("Self");
                                globalInfo.basicgauges[i]._gaugeGainOnHit = StyledSlider("Gauge Gain on Hit (%)", (float)globalInfo.basicgauges[i]._gaugeGainOnHit, EditorGUI.indentLevel, 0, 100);
                                globalInfo.basicgauges[i]._gaugeGainOnBlock = StyledSlider("Gauge Gain on Block (%)", (float)globalInfo.basicgauges[i]._gaugeGainOnBlock, EditorGUI.indentLevel, 0, 100);
                                globalInfo.basicgauges[i]._gaugeRequired = StyledSlider("Gauge Required (%)", (float)globalInfo.basicgauges[i]._gaugeRequired, EditorGUI.indentLevel, 0, 100);
                                globalInfo.basicgauges[i]._gaugeUsage = StyledSlider("Gauge Cost (%)", (float)globalInfo.basicgauges[i]._gaugeUsage, EditorGUI.indentLevel, 0, 100);

                                EditorGUILayout.Space();
                                SubGroupTitle("Opponent");
                                globalInfo.basicgauges[i]._opGaugeGainOnHit = StyledSlider("Gauge Gain on Hit (%)", (float)globalInfo.basicgauges[i]._opGaugeGainOnHit, EditorGUI.indentLevel, 0, 100);
                                globalInfo.basicgauges[i]._opGaugeGainOnBlock = StyledSlider("Gauge Gain on Block (%)", (float)globalInfo.basicgauges[i]._opGaugeGainOnBlock, EditorGUI.indentLevel, 0, 100);
                                globalInfo.basicgauges[i]._opGaugeGainOnParry = StyledSlider("Gauge Gain on Parry (%)", (float)globalInfo.basicgauges[i]._opGaugeGainOnParry, EditorGUI.indentLevel, 0, 100);
                        ...
                        if (StyledButton("New Gauge Options"))
                            globalInfo.basicgauges = AddElement<GaugeInfo>(globalInfo.basicgauges, new GaugeInfo());
            ...
            EditorGUILayout.BeginVertical(rootGroupStyle);
            {
                EditorGUILayout.BeginHorizontal();
                {
                    characterOptions = EditorGUILayout.Foldout(characterOptions, "Characters (" + globalInfo.skins.Length + ")", foldStyle);
                ...
                if (characterOptions)
                {
                    EditorGUILayout.BeginVertical(subGroupStyle);
                    {
                        ...
                        for (int i = 0; i < globalInfo.skins.Length; i++)
                        {
                            EditorGUILayout.Space();
                            EditorGUILayout.BeginVertical(arrayElementStyle);
                            {
                                ...
                                EditorGUILayout.BeginHorizontal();
                                {
                                    EditorGUILayout.BeginVertical(subGroupStyle);
                                    {
                                        ...
                                        for (int j = 0; j < globalInfo.skins[i].characters.Length; j++)
                                        {
                                            EditorGUILayout.Space();
                                            EditorGUILayout.BeginVertical(arrayElementStyle);
                                            {
                                                ...
                                                EditorGUILayout.BeginHorizontal();
                                                {
                                                    globalInfo.skins[i].characters[j] = (TOHDragonFight3D.CharacterInfo)EditorGUILayout.ObjectField("Character File:", globalInfo.skins[i].characters[j], typeof(TOHDragonFight3D.CharacterInfo), false);
                                                    if (GUILayout.Button("", "PaneOptions"))
                                                    {
                                                        PaneOptions<TOHDragonFight3D.CharacterInfo>(
                                                            globalInfo.skins[i].characters,
                                                            globalInfo.skins[i].characters[j],
                                                            delegate (TOHDragonFight3D.CharacterInfo[] newElement)
                                                            {
                                                                globalInfo.skins[i].characters = newElement;
                                                            }
                                                        );
                                                    }
                                                }
                                                ...
                                                {
                                                    CharacterEditorWindow.sentCharacterInfo = globalInfo.skins[i].characters[j];
                                        ...
                                        {
                                            globalInfo.skins[i].characters = AddElement<TOHDragonFight3D.CharacterInfo>(globalInfo.skins[i].characters, null);
                                        }
                                    }
                                    EditorGUILayout.EndVertical();
                                }
                                ...
                                if (GUILayout.Button("", "PaneOptions"))
                                {
                                    PaneOptions<TOHDragonFight3D.FullSkinInfo>(
                                        globalInfo.skins,
                                        globalInfo.skins[i],
                                        delegate (TOHDragonFight3D.FullSkinInfo[] newElement)
                                        {
                                            globalInfo.skins = newElement;
                                        }
                                    );
                                }
                        ...
                        EditorGUILayout.Space();
                        if (StyledButton("New Full-Skin"))
                        {
                            globalInfo.skins = AddElement<TOHDragonFight3D.FullSkinInfo>(globalInfo.skins, null);
                            globalInfo.storyMode.selectableCharactersInStoryMode.Add(globalInfo.skins.Length - 1);
                            globalInfo.storyMode.selectableCharactersInVersusMode.Add(globalInfo.skins.Length - 1);
                            globalInfo.ValidateStoryModeInformation();
                        }
                    }
                    EditorGUILayout.EndVertical();
                            ...
                            if (storyModeSelectableCharactersInStoryModeOptions)
                            {
                                ...
                                for (int i = 0; i < globalInfo.skins.Length; i++)
                                {
                                    ...
                                    EditorGUILayout.BeginVertical(arrayElementStyle);
                                    {
                                        ...
                                        {
                                            string name;
                                            if (globalInfo.skins[i] != null && globalInfo.skins[i].characters.Length > 0)
                                            {
                                                name = globalInfo.skins[i].characters[0].characterName;
                                            }
                                            else
                                            {
                                                name = "Character " + i;
                    ...
                    EditorGUILayout.BeginVertical(subGroupStyle);
                    {
                        ...
                        for (int i = 0; i < globalInfo.challengeModeOptions.Length; i++)
                        {
                            ...
                            EditorGUILayout.BeginVertical(arrayElementStyle);
                            {
                                ...
                                globalInfo.challengeModeOptions[i].character = (TOHDragonFight3D.CharacterInfo)EditorGUILayout.ObjectField("Character (P1):", globalInfo.challengeModeOptions[i].character, typeof(TOHDragonFight3D.CharacterInfo), false);
                                globalInfo.challengeModeOptions[i].opCharacter = (TOHDragonFight3D.CharacterInfo)EditorGUILayout.ObjectField("Opponent (P2):", globalInfo.challengeModeOptions[i].opCharacter, typeof(TOHDragonFight3D.CharacterInfo), false);
    ...
    public void FightGroupsBlock(FightsGroup[] fights, Action<FightsGroup[]> callback)
    {
        ...
        for (int i = 0; i < globalInfo.skins.Length; ++i)
        {
            if (globalInfo.skins[i] != null && globalInfo.skins[i].characters.Length > 0)
            {
                characterNames.Add(globalInfo.skins[i].characters[0].name);
            }
            else
            {
    ...
    public void StoryModeBattleBlock(FightsGroup[] fights, int fightIndex, string[] characterNames, Action<FightsGroup[]> callback)
    {
        this.StoryModeBattleBlock(fights[fightIndex].opponents, characterNames, delegate (StoryModeBattle[] obj)
        {
            ...
            {
                fights[fightIndex].opponents[previousLength].opponentCharacterIndex = (previousLength % globalInfo.skins.Length);
