...
{
    [Serializable] public class FullSkinInfo { public CharacterInfo[] characters = new CharacterInfo[0]; }
    [System.Serializable]
    public class GlobalInfo : ScriptableObject
    {
        ...
        public GaugeInfo[] basicgauges = new GaugeInfo[0];
        ...
        public FullSkinInfo[] skins = new FullSkinInfo[0];
        ...
        #region temporary editor variable
        ...
        public virtual void ValidateStoryModeInformation()
        {
            ...
            for (int i = this.storyMode.selectableCharactersInStoryMode.Count - 1; i >= 0; --i)
            {
                ...
                if (character < 0 || character >= this.skins.Length)
                {
            ...
            if (story != null && story.fightsGroups != null)
            {
                foreach (FightsGroup group in story.fightsGroups)
                {
                    ...
                    for (int i = battles.Count - 1; i >= 0; --i)
                    {
                        ...
                        if (battle.opponentCharacterIndex < 0 || battle.opponentCharacterIndex >= this.skins.Length)
                        {
                            battles.RemoveAt(i);
                        }
                        else
