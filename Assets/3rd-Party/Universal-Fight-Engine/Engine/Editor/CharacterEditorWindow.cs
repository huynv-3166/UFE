...
public class CharacterEditorWindow : EditorWindow
{
    ...
    public static TOHDragonFight3D.CharacterInfo sentCharacterInfo;
    private TOHDragonFight3D.CharacterInfo characterInfo;
    ...
    private bool personalInfoOption;
    private bool statsInfoOption;
    ...
    void Populate()
    {
        ...
        UnityEngine.Object[] selection = Selection.GetFiltered(typeof(TOHDragonFight3D.CharacterInfo), SelectionMode.Assets);
        if (selection.Length > 0)
        {
            if (selection[0] == null) return;
            characterInfo = (TOHDragonFight3D.CharacterInfo)selection[0];
        }
    }
    ...
    public void OnGUI()
    {
        if (characterInfo == null)
        {
            ...
            if (GUILayout.Button("Create new character"))
                ScriptableObjectUtility.CreateAsset<TOHDragonFight3D.CharacterInfo>();
            return;
        }
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    ...
                    EditorGUILayout.BeginVertical();
                    {
                        ...
                        EditorGUILayout.BeginVertical(rootGroupStyle);
                        {
                            EditorGUILayout.BeginHorizontal();
                            {
                                personalInfoOption = EditorGUILayout.Foldout(personalInfoOption, "Basic Info", foldStyle);
                            ...
                            if (personalInfoOption)
                            {
                                EditorGUILayout.BeginVertical();
                                {
                                    ...
                                    characterInfo.levelPoints = EditorGUILayout.IntField("Level:", characterInfo.levelPoints);
                                }
                                EditorGUILayout.EndVertical();
                            }
                        }
                        ...
                        EditorGUILayout.BeginVertical(rootGroupStyle);
                        {
                            EditorGUILayout.BeginHorizontal();
                            {
                                statsInfoOption = EditorGUILayout.Foldout(statsInfoOption, "Statistics Info", foldStyle);
                                helpButton("character:prefabs");
                            }
                            EditorGUILayout.EndHorizontal();
                            if (statsInfoOption)
                            {
                                EditorGUILayout.BeginVertical();
                                {
                                    characterInfo.healthPoints = EditorGUILayout.IntField("HP:", characterInfo.healthPoints);
                                    characterInfo.attackPoints = EditorGUILayout.IntField("ATK:", characterInfo.attackPoints);
                                    characterInfo.maxEPS = EditorGUILayout.IntField("EPS:", characterInfo.maxEPS);
                                    characterInfo.luckPoints = EditorGUILayout.IntField("LUCK:", characterInfo.luckPoints);
                                    characterInfo.critPoints = EditorGUILayout.IntField("CRI dmg:", characterInfo.critPoints);
                                    characterInfo.speedPoints = EditorGUILayout.IntField("SPD:", characterInfo.speedPoints);
                                    characterInfo.reducedamagePoints = EditorGUILayout.IntField("Reduce Damage:", characterInfo.reducedamagePoints);

                                    for (int i = 0; i < characterInfo.natures.Length; i++)
                                    {
                                        EditorGUILayout.Space();
                                        EditorGUILayout.BeginVertical(arrayElementStyle);
                                        {
                                            ...
                                            EditorGUILayout.BeginHorizontal();
                                            {
                                                characterInfo.natures[i] = (CharacterNatureInfo)EditorGUILayout.ObjectField("Nature:", characterInfo.natures[i], typeof(CharacterNatureInfo), true);
                                                if (GUILayout.Button("", "PaneOptions"))
                                                {
                                                    PaneOptions<CharacterNatureInfo>(characterInfo.natures, characterInfo.natures[i], delegate (CharacterNatureInfo[] newElement) { characterInfo.natures = newElement; });
                                                }
                                            }
                                            EditorGUILayout.EndHorizontal();
                                        }
                                    ...
                                    if (StyledButton("New Nature Link"))
                                        characterInfo.natures = AddElement<CharacterNatureInfo>(characterInfo.natures, null);
                        ...
                        // Rareness and attributes
                        EditorGUILayout.BeginVertical(rootGroupStyle);
                        {
                            EditorGUILayout.BeginHorizontal();
                            {
                                rarenessOption = EditorGUILayout.Foldout(rarenessOption, "Rareness and attributes", foldStyle);
                            ...
                            if (rarenessOption)
                            {
                                EditorGUILayout.BeginVertical(subGroupStyle);
                                {
                                    ...
                                    characterInfo.rareness = EditorGUILayout.IntField("Rareness:", characterInfo.rareness);

                                    for (int i = 0; i < characterInfo.characterAtrributes.Length; i++)
                                    {
                                        EditorGUILayout.Space();
                                        EditorGUILayout.BeginVertical(arrayElementStyle);
                                        {
                                            ...
                                            EditorGUILayout.BeginHorizontal();
                                            {
                                                characterInfo.characterAtrributes[i] = (CharacterAttributeInfo)EditorGUILayout.ObjectField("Attribute:", characterInfo.characterAtrributes[i], typeof(CharacterAttributeInfo), true);
                                                if (GUILayout.Button("", "PaneOptions"))
                                                {
                                                    PaneOptions<CharacterAttributeInfo>(characterInfo.characterAtrributes, characterInfo.characterAtrributes[i], delegate (CharacterAttributeInfo[] newElement) { characterInfo.characterAtrributes = newElement; });
                                                }
                                            }
                                            EditorGUILayout.EndHorizontal();
                                        }
                                    ...
                                    if (StyledButton("New Effectiveness Link"))
                                        characterInfo.characterAtrributes = AddElement<CharacterAttributeInfo>(characterInfo.characterAtrributes, null);
            ...
            EditorGUILayout.BeginVertical(rootGroupStyle);
            {
                ...
                if (statusResistanceOption)
                {
                    EditorGUILayout.BeginVertical(subGroupStyle);
                    {
                        ...
                        for (int i = 0; i < characterInfo.statusResistances.Length; i++)
                        {
                            EditorGUILayout.Space();
                            EditorGUILayout.BeginVertical(arrayElementStyle);
                            {
                                ...
                                EditorGUILayout.BeginHorizontal();
                                {
                                    characterInfo.statusResistances[i] = (StatusResistanceInfo)EditorGUILayout.ObjectField("Status Resistance:", characterInfo.statusResistances[i], typeof(StatusResistanceInfo), true);
                                    if (GUILayout.Button("", "PaneOptions"))
                                    {
                                        PaneOptions<StatusResistanceInfo>(characterInfo.statusResistances, characterInfo.statusResistances[i], delegate (StatusResistanceInfo[] newElement) { characterInfo.statusResistances = newElement; });
                                    }
                                }
                                EditorGUILayout.EndHorizontal();
                            }
                        ...
                        if (StyledButton("New Status Resistance Link"))
                            characterInfo.statusResistances = AddElement<StatusResistanceInfo>(characterInfo.statusResistances, null);
    ...
    public void StanceBlock(MoveSetData moveSet, bool resource = false)
    {
        EditorGUILayout.BeginVertical(resource ? subArrayElementStyle : arrayElementStyle);
        {
            ...
            moveSet.skillMovesToggle = EditorGUILayout.Foldout(moveSet.skillMovesToggle, "Skill Moves (" + moveSet.skillMoves.Length + ")", foldStyle);
            if (moveSet.skillMovesToggle)
            {
                EditorGUILayout.BeginVertical(subGroupStyle);
                {
                    EditorGUILayout.Space();
                    ...
                    for (int z = 0; z < moveSet.skillMoves.Length; z++)
                    {
                        EditorGUILayout.Space();
                        EditorGUILayout.BeginVertical(subArrayElementStyle);
                        {
                            ...
                            EditorGUILayout.BeginHorizontal();
                            {
                                moveSet.skillMoves[z] = (MoveInfo)EditorGUILayout.ObjectField("Move File:", moveSet.skillMoves[z], typeof(MoveInfo), false);
                                if (GUILayout.Button("", "PaneOptions"))
                                {
                                    PaneOptions<MoveInfo>(moveSet.skillMoves, moveSet.skillMoves[z], delegate (MoveInfo[] newElement) { moveSet.skillMoves = newElement; });
                                }
                            }
                            EditorGUILayout.EndHorizontal();
                            EditorGUIUtility.labelWidth = 150;
                            ...
                            if (GUILayout.Button("Open in the Move Editor"))
                            {
                                MoveEditorWindow.sentMoveInfo = moveSet.skillMoves[z];
                                MoveEditorWindow.Init();
                            }
                        }
                        EditorGUILayout.EndVertical();
                    ...
                    EditorGUILayout.Space();
                    if (StyledButton("New Move"))
                        moveSet.skillMoves = AddElement<MoveInfo>(moveSet.skillMoves, null);
