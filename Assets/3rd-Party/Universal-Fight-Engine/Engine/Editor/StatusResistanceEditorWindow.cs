...
public class StatusResistanceEditorWindow : EditorWindow
{
    public static StatusResistanceEditorWindow statusresistanceEditorWindow;
    public static TOHDragonFight3D.StatusResistanceInfo sentStatusResistanceInfo;
    private TOHDragonFight3D.StatusResistanceInfo statusInfo;
    ...
    [MenuItem("Window/U.F.E./Status Resistance Editor")]
    public static void Init()
    {
        statusresistanceEditorWindow = EditorWindow.GetWindow<StatusResistanceEditorWindow>(false, "Character", true);
        statusresistanceEditorWindow.Show();
        statusresistanceEditorWindow.Populate();
    }
    ...
    void Populate()
    {
        this.titleContent = new GUIContent("Status Resistance", (Texture)Resources.Load("Icons/Square"));
        ...
        if (sentStatusResistanceInfo != null)
        {
            EditorGUIUtility.PingObject(sentStatusResistanceInfo);
            Selection.activeObject = sentStatusResistanceInfo;
            sentStatusResistanceInfo = null;
        }

        UnityEngine.Object[] selection = Selection.GetFiltered(typeof(TOHDragonFight3D.StatusResistanceInfo), SelectionMode.Assets);
        if (selection.Length > 0)
        {
            if (selection[0] == null) return;
            statusInfo = (TOHDragonFight3D.StatusResistanceInfo)selection[0];
        }
    }
    ...
    public void OnGUI()
    {
        if (statusInfo == null)
        {
            ...
            GUILayout.Label("Select a stats resistance file\nor create a new stat resistance.", "CN EntryInfo");
            GUILayout.EndHorizontal();
            EditorGUILayout.Space();
            if (GUILayout.Button("Create new stats resistance"))
                ScriptableObjectUtility.CreateAsset<TOHDragonFight3D.StatusResistanceInfo>();
            return;
        }
        ...
        EditorGUILayout.BeginVertical(titleStyle);
        {
            EditorGUILayout.BeginHorizontal();
            {
                ...
                EditorGUILayout.LabelField("", statusInfo.resistanceName == "" ? "New Character" : statusInfo.resistanceName, fontStyle, GUILayout.Height(32));
#else
        ...
        scrollPos = EditorGUILayout.BeginScrollView(scrollPos);
        {
            EditorGUILayout.BeginVertical(rootGroupStyle);
            {
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    statusInfo.resistancePicture = (Sprite)EditorGUILayout.ObjectField(statusInfo.resistancePicture, typeof(Sprite), false, GUILayout.Width(100), GUILayout.Height(100));
                    ...
                    EditorGUILayout.BeginVertical(rootGroupStyle);
                    {
                        ...
                        if (basicInfoOption)
                        {
                            EditorGUILayout.BeginVertical();
                            {
                                ...
                                statusInfo.resistanceName = EditorGUILayout.TextField("Name:", statusInfo.resistanceName);
                                statusInfo.resistanceGroup = (StatusResistanceGroup)EditorGUILayout.EnumPopup("Group:", statusInfo.resistanceGroup);
                            }
                            EditorGUILayout.EndVertical();
                        }
                    }
                ...
                statusInfo.resistanceDescription = EditorGUI.TextArea(rect, statusInfo.resistanceDescription);
            ...
            EditorGUILayout.BeginVertical(rootGroupStyle);
            {
                ...
                if (effectiveOptions)
                {
                    EditorGUILayout.BeginVertical(subGroupStyle);
                    {
                        ...
                        //    for (int i = 0; i < resistanceInfo.effectLinks.Length; i++)
                        //    {
                        //        EditorGUILayout.Space();
                        ...
                        //            EditorGUILayout.BeginHorizontal();
                        //            {
                        //                resistanceInfo.effectLinks[i].targetattributeType = (CharacterAtrributeType)EditorGUILayout.EnumPopup("Target:", resistanceInfo.effectLinks[i].targetattributeType, enumStyle);
                        //                if (GUILayout.Button("", "PaneOptions"))
                        //                {
                        //                    PaneOptions<Effectiveness>(resistanceInfo.effectLinks, resistanceInfo.effectLinks[i], delegate (Effectiveness[] newElement) { resistanceInfo.effectLinks = newElement; });
                        //                }
                        //            }
                        //            EditorGUILayout.EndHorizontal();
                        //            resistanceInfo.effectLinks[i].effectiveness = EditorGUILayout.FloatField("", resistanceInfo.effectLinks[i].effectiveness);
                        //        }
                        //        EditorGUILayout.EndVertical();
                        //    }
                        //    if (StyledButton("New Effectiveness Link"))
                        //        resistanceInfo.effectLinks = AddElement<Effectiveness>(resistanceInfo.effectLinks, null);
            ...
            EditorGUILayout.BeginVertical(rootGroupStyle);
            {
                EditorGUILayout.BeginHorizontal();
                {
                    particleEffectsOptions = EditorGUILayout.Foldout(particleEffectsOptions, "Particle Effects (" + statusInfo.particleEffects.Length + ")", EditorStyles.foldout);
                ...
                if (particleEffectsOptions)
                {
                    ...
                    {
                        EditorGUI.indentLevel += 1;
                        statusInfo.maxCastingFrame = EditorGUILayout.IntField("Max Casting Frames:", Mathf.Max(statusInfo.maxCastingFrame, 0)); ;
                        List<int> castingValues = new List<int>();
                        foreach (AbstractParticleEffect particleEffect in statusInfo.particleEffects)
                            castingValues.Add(particleEffect.castingFrame);
                        StyledMarker("Casting Timeline", castingValues.ToArray(), statusInfo.maxCastingFrame, EditorGUI.indentLevel);

                        for (int i = 0; i < statusInfo.particleEffects.Length; i++)
                        {
                            EditorGUILayout.Space();
                            EditorGUILayout.BeginVertical(arrayElementStyle);
                            {
                                ...
                                EditorGUILayout.BeginHorizontal();
                                {
                                    statusInfo.particleEffects[i].castingFrame = EditorGUILayout.IntSlider("Casting Frame:", statusInfo.particleEffects[i].castingFrame, 0, statusInfo.maxCastingFrame);
                                    if (GUILayout.Button("", "PaneOptions"))
                                    {
                                        PaneOptions<AbstractParticleEffect>(statusInfo.particleEffects, statusInfo.particleEffects[i], delegate (AbstractParticleEffect[] newElement) { statusInfo.particleEffects = newElement; });
                                    }
                                }
                                EditorGUILayout.EndHorizontal();
                                if (statusInfo.particleEffects[i].particleEffect == null) statusInfo.particleEffects[i].particleEffect = new ParticleInfo();
                                statusInfo.particleEffects[i].particleEffect.prefab = (GameObject)EditorGUILayout.ObjectField("Particle Effect:", statusInfo.particleEffects[i].particleEffect.prefab, typeof(UnityEngine.GameObject), true);
                                statusInfo.particleEffects[i].particleEffect.identity = (IdentityType)EditorGUILayout.EnumPopup("Identity:", statusInfo.particleEffects[i].particleEffect.identity, enumStyle);

                                EditorGUIUtility.labelWidth = 180;
                                statusInfo.particleEffects[i].particleEffect.stick = EditorGUILayout.Toggle("Sticky", statusInfo.particleEffects[i].particleEffect.stick, toggleStyle);
                                statusInfo.particleEffects[i].particleEffect.destroyOnMoveOver = EditorGUILayout.Toggle("Destroy when move ends", statusInfo.particleEffects[i].particleEffect.destroyOnMoveOver, toggleStyle);
                                EditorGUI.BeginDisabledGroup(statusInfo.particleEffects[i].particleEffect.destroyOnMoveOver);
                                {
                                    statusInfo.particleEffects[i].particleEffect.duration = EditorGUILayout.FloatField("Duration (seconds):", statusInfo.particleEffects[i].particleEffect.duration);
                                }
                                EditorGUI.EndDisabledGroup();

                                if (statusInfo.particleEffects[i].particleEffect.stick)
                                    statusInfo.particleEffects[i].particleEffect.followRotation = EditorGUILayout.Toggle("Follow Rotation", statusInfo.particleEffects[i].particleEffect.followRotation, toggleStyle);
                                EditorGUIUtility.labelWidth = 150;

                                statusInfo.particleEffects[i].particleEffect.bodyPart = (BodyPart)EditorGUILayout.EnumPopup("Body Part:", statusInfo.particleEffects[i].particleEffect.bodyPart, enumStyle);

                                statusInfo.particleEffects[i].particleEffect.overrideRotation = EditorGUILayout.Toggle("Override Prefab Rotation", statusInfo.particleEffects[i].particleEffect.overrideRotation, toggleStyle);
                                if (statusInfo.particleEffects[i].particleEffect.overrideRotation)
                                    statusInfo.particleEffects[i].particleEffect.initialRotation = EditorGUILayout.Vector3Field("Initial Rotation:", statusInfo.particleEffects[i].particleEffect.initialRotation);

                                statusInfo.particleEffects[i].particleEffect.positionOffSet = EditorGUILayout.Vector3Field("Off Set (Position):", statusInfo.particleEffects[i].particleEffect.positionOffSet);
                                statusInfo.particleEffects[i].particleEffect.lockLocalPosition = EditorGUILayout.Toggle("Lock Local Position", statusInfo.particleEffects[i].particleEffect.lockLocalPosition, toggleStyle);
                                statusInfo.particleEffects[i].particleEffect.mirrorOn2PSide = EditorGUILayout.Toggle("Mirror on Right Side:", statusInfo.particleEffects[i].particleEffect.mirrorOn2PSide, toggleStyle);
                        ...
                        if (StyledButton("New Particle Effect"))
                            statusInfo.particleEffects = AddElement<AbstractParticleEffect>(statusInfo.particleEffects, new AbstractParticleEffect());
            ...
            // Begin Ability Options
            EditorGUILayout.BeginVertical(rootGroupStyle);
            {
                EditorGUILayout.BeginHorizontal();
                {
                    abilityOptions = EditorGUILayout.Foldout(abilityOptions, "Status Ability", EditorStyles.foldout);
                    helpButton("move:particleeffects");
                }
                EditorGUILayout.EndHorizontal();
                ...
                if (abilityOptions)
                {
                    EditorGUILayout.BeginVertical(subGroupStyle);
                    {
                        statusInfo.duration = EditorGUILayout.FloatField("Duration:", (float)statusInfo.duration);

                        statusInfo.canReduceHp = EditorGUILayout.Toggle("Reduce HP by %", statusInfo.canReduceHp, toggleStyle);
                        if (statusInfo.canReduceHp)
                            statusInfo.hpReduce = EditorGUILayout.FloatField("HP reduce:", (float)statusInfo.hpReduce);

                        statusInfo.canSlow = EditorGUILayout.Toggle("Slow speed by %", statusInfo.canSlow, toggleStyle);
                        if (statusInfo.canSlow)
                            statusInfo.slowStats = EditorGUILayout.FloatField("Slow stats:", (float)statusInfo.slowStats);
                    }
                    EditorGUILayout.EndVertical();
                }
        ...
        if (GUI.changed)
        {
            Undo.RecordObject(statusInfo, "Character Editor Modify");
            EditorUtility.SetDirty(statusInfo);
