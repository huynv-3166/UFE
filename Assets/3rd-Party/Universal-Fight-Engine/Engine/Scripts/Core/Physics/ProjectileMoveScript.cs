...
namespace TOHDragonFight3D
{
    public class ProjectileMoveScript : UFEBehaviour, UFEInterface
    {
        ...
        public void Init(MoveInfo move)
        {
            ...
            if (data.characterScaling)
            {
                data.attributeEffectiveness = UFE.CheckAttribute(myControlsScript, opControlsScript);
                data.stab = UFE.CheckStab(initMove, myControlsScript);
            }

            if (initMove != null)
                data.statusResistances = initMove.statusResistances;

            // Create Hit data
            hit = new Hit();
            hit.initMove = initMove;
            hit.hitType = data.hitType;
            hit.attributeEffectiveness = data.attributeEffectiveness;
            hit.stab = data.stab;
            hit.statusResistances = data.statusResistances;
            ...
            hit.characterScaling = data.characterScaling;
