...
public class DefaultCharacterSelectionScreen : CharacterSelectionScreen
{
    ...
    public override void SetHoverIndex(int player, int skinIndex)
    {
        if (!this.closing)
        {
            ...
            base.SetHoverIndex(player, skinIndex);

            if (skinIndex >= 0 && skinIndex <= maxCharacterIndex)
            {
                ...
                if (player == 1)
                {
                    ...
                    {
                        TOHDragonFight3D.CharacterInfo characterInfo = UFE.config.skins[skinIndex].characters[0];
    ...
    public override void OnShow()
    {
        ...
        mainCam.transform.eulerAngles = UFE.config.cameraOptions.initialRotation;
        ...
        if (this.characters != null)
        {
            ...
            for (int i = 0; i < this.selectableCharacters.Length && i < this.characters.Length; ++i)
            {
                ...
                TOHDragonFight3D.CharacterInfo selectableCharacter = this.selectableCharacters[i];
    ...
    #region public methods for calling in GUI
    public void NextSkin(int player)
    {
        Debug.Log("Changing skin player " + player);
    }
    #endregion
}
