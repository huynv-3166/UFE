...
public class ControlsScript : MonoBehaviour
{
    ...
    public TOHDragonFight3D.CharacterInfo myInfo;
    public int mirror;
    ...
    public NatureScript Natures { get { return this.myNaturesScript; } set { myNaturesScript = value; } }
    ...
    public bool IsReadingSkill = false;

    [HideInInspector] public GameObject character;
    [HideInInspector] public TOHDragonFight3D.CharacterInfo opInfo;
    ...
    private NatureScript myNaturesScript;
    ...
    // Blinking red
    private void InitBleedEffect()
    {
        Color bleedVFXColor = Color.red;
        Renderer[] charRenderers = character.GetComponentsInChildren<Renderer>();
        foreach (Renderer char_rend in charRenderers)
        {
            Array.ForEach(char_rend.materials,
                material => material.SetColor("_Color", bleedVFXColor));
        }
    }
    ...
    )
    {
        ...
        if (!isAssist)
        {
            ...
            if ((UFE.gameMode == GameMode.TrainingRoom || UFE.gameMode == GameMode.ChallengeMode)
                && currentGaugesPoints[0] < myInfo.maxEPS
            ...
            if ((UFE.gameMode == GameMode.TrainingRoom || UFE.gameMode == GameMode.ChallengeMode)
                && currentLifePoints < myInfo.healthPoints
        ...
        if (gaugeDPS != 0)
        {
            owner.currentGaugesPoints[(int)gaugeDrainId] -= (owner.myInfo.maxEPS * (gaugeDPS / 100) / UFE.config.fps) * UFE.timeScale;
                    ...
                    if (myMoveSetScript.AnimationExists(myMoveSetScript.basicMoves.takeOff.name))
                    {
                        ...
                        if (myMoveSetScript.basicMoves.takeOff.autoSpeed)
                        {
                            myMoveSetScript.SetAnimationSpeed(
                                ...
                                //myMoveSetScript.basicMoves.takeOff.scaleCharacterSpeed ? myInfo.speedPoints : 100f);
                                );
                        }
                    }
                }
        ...
        if (standUpAnimation != null && !myMoveSetScript.IsAnimationPlaying(standUpAnimation))
        {
            ...
            if (myMoveSetScript.basicMoves.standUp.autoSpeed)
            {
                myMoveSetScript.SetAnimationSpeed(standUpAnimation,
                                                  ...
                                                  //newBasicMove.scaleCharacterSpeed ? myInfo.speedPoints : 100f);
                                                  );
            }
    ...
    public void SetMove(MoveInfo move)
    {
        ...
        if (currentMove != null)
        {
            ...
            myMoveSetScript.SetAnimationSpeed(move.name,
                                              move._animationSpeed
                                              //move.scaleCharacterSpeed ? myInfo.speedPoints : 100f);
                                              );
    ...
    public void ReadMove(MoveInfo move)
    {
        ...
        if (loadedMoves.Length > 0 &&
            Array.Exists(loadedMoves, moveset =>
            Array.Exists(moveset.skillMoves, skill => skill.moveName == move.moveName) == true) == true)
        {
            IsReadingSkill = true;
        }
        else
        ...
        if (!move.fixedSpeed)
        {
            foreach (AnimSpeedKeyFrame speedKeyFrame in move.animSpeedKeyFrame)
            {
                ...
                {
                    myMoveSetScript.SetAnimationSpeed(move.name,
                                                      speedKeyFrame._speed * move._animationSpeed
                                                      //move.scaleCharacterSpeed ? myInfo.speedPoints : 100f);
                                                      );
                }
            }
        ...
        foreach (Projectile projectile in move.projectiles)
        {
            ...
            )
            {
                ...
                projectileMoveScript.Init(move);
        ...
        int partCount = 0;
        foreach (AbstractParticleEffect particleEffect in move.particleEffects)
        {
            if (
        ...
        foreach (AppliedForce addedForce in move.appliedForces)
        {
            if (!addedForce.casted && move.currentFrame >= addedForce.castingFrame)
            {
                ...
                if (!addedForce.towardOpponent && !addedForce.backwardOpponent)
                    myPhysicsScript.AddForce(addedForce._force, GetDirection(), true);
                else
                {
                    opControlsScript = UFE.GetControlsScript(playerNum == 1 ? 2 : 1);
                    Vector3 distance = opControlsScript.transform.position - this.transform.position;
                    Vector3 v3force = addedForce._force.ToVector();
                    Vector3 directionalForce = Vector3.zero;
                    if (addedForce.towardOpponent)
                    {
                        directionalForce = new Vector3(Mathf.Abs(distance.x * v3force.x),
                            Mathf.Abs(distance.y * v3force.y), Mathf.Abs(distance.z * v3force.z));
                    }
                    else if (addedForce.backwardOpponent)
                    {
                        directionalForce = new Vector3(Mathf.Abs(-distance.x * v3force.x),
                            Mathf.Abs(distance.y * v3force.y), Mathf.Abs(distance.z * v3force.z));
                    }
                    myPhysicsScript.AddForce(FPVector.ToFPVector(directionalForce), GetDirection(), true);
                }
        ...
        {
            if (!opponentOverride.casted &&
                move.currentFrame >= opponentOverride.castingFrame)
            {
                ...
                foreach (CharacterSpecificMoves csMove in opponentOverride.characterSpecificMoves)
                {
                    if (opInfo.characterName == csMove.characterName &&
                        (myMoveSetScript != null && myMoveSetScript.ValidateMoveStances(csMove.conditions, opControlsScript)))
                    {
        ...
        if (move.currentFrame >= move.totalFrames - 1)
        {
            ...
            KillCurrentMove();
            if (IsReadingSkill) IsReadingSkill = false;
    ...
    public void CheckHits(MoveInfo move, ControlsScript opControlsScript)
    {
        ...
        //Check every time the hit does hit
        //TO-DO : Fix it by checking only at Init()
        float currentAE = 0f;
        //UFE.CheckAttribute(this, opControlsScript);
        float currentStab = 0f;
        //UFE.CheckStab(move, opControlsScript);

        HurtBox[] activeHurtBoxes = null;
        for (int i = 0; i < move.hits.Length; i++)
        {
            Hit hit = move.hits[i];
            hit.statusResistances = new StatusResistanceInfo[move.statusResistances.Length];
            Array.Copy(move.statusResistances, hit.statusResistances, move.statusResistances.Length);
            Array.ForEach(hit.statusResistances,
                status => Array.ForEach(status.particleEffects,
                vfx => vfx.casted = false));
            hit.attributeEffectiveness = currentAE;
            hit.stab = currentStab;
            hit.characterScaling = move.characterScaling;
            hit.autoAbility = move.autoAbility;
    ...
    public void GetHitParry(Hit hit, int remainingFrames, FPVector[] location, ControlsScript attacker)
    {
        ...
        if (currentHitInfo.autoSpeed)
        {
            myMoveSetScript.SetAnimationSpeed(currentHitAnimation,
                                              ...
                                              //currentHitInfo.scaleCharacterSpeed ? myInfo.speedPoints : 100f);
                                              );
        }
    ...
    public void GetHitBlocking(Hit hit, int remainingFrames, FPVector[] location, bool ignoreDirection, ControlsScript attacker)
    {
        ...
        else
        {
            ...
            if (hit.damageType == DamageType.Percentage) damage = myInfo.healthPoints * (damage / 100);
            DamageMe(damage);
        }
    ...
    public void GetHit(Hit hit, int remainingFrames, FPVector[] location, bool ignoreDirection, ControlsScript attacker)
    {
        ...
        foreach (StatusResistanceInfo status in hit.statusResistances)
        {
            // Create status particles effect
            int partCount = 0;
            foreach (AbstractParticleEffect particleEffect in status.particleEffects)
            {
                ...
                )
                {
                    ...
                    // Checking whether identity is Self or Opponent
                    HitBoxesScript hitBoxesScript = null;
                    if (particleEffect.particleEffect.identity == IdentityType.self)
                        hitBoxesScript = attacker.myHitBoxesScript;
                    else if (particleEffect.particleEffect.identity == IdentityType.opponent)
                        hitBoxesScript = myHitBoxesScript;

                    if (hitBoxesScript != null)
                    {
                        UFE.FireParticleEffects(currentMove, this, particleEffect);

                        long duration = Mathf.RoundToInt(particleEffect.particleEffect.duration * UFE.config.fps);
                        Quaternion newRotation = particleEffect.particleEffect.initialRotation != Vector3.zero ? Quaternion.Euler(particleEffect.particleEffect.initialRotation) : Quaternion.identity;
                        string uniqueId = particleEffect.particleEffect.prefab.name + playerNum.ToString() + UFE.currentFrame + partCount;
                        GameObject pTemp = UFE.SpawnGameObject(particleEffect.particleEffect.prefab, Vector3.zero, newRotation, duration, false, uniqueId);
                        if (!particleEffect.particleEffect.overrideRotation)
                            pTemp.transform.rotation = particleEffect.particleEffect.prefab.transform.rotation;

                        if (particleEffect.particleEffect.stick)
                        {
                            Transform targetTransform = hitBoxesScript.GetTransform(particleEffect.particleEffect.bodyPart);
                            pTemp.transform.SetParent(targetTransform);
                            pTemp.transform.position = targetTransform.position;
                            if (particleEffect.particleEffect.followRotation) pTemp.AddComponent<StickyGameObject>();
                        }
                        ...
                        {
                            pTemp.transform.SetParent(UFE.gameEngine.transform);
                            pTemp.transform.position = hitBoxesScript.GetPosition(particleEffect.particleEffect.bodyPart).ToVector();
                        }
            ...
            if (status.canReduceHp)
            {
                Bleed bleed = gameObject.AddComponent<Bleed>();
                bleed.Init(this, status, true);
                bleed.Run();
            }

            if (status.canSlow)
            {
                Slow slow = gameObject.AddComponent<Slow>();
                slow.Init(this, status, true);
                slow.Run();
            }
        }
        #endregion Status Resistance
        ...
        // Character Scaling
        if (hit.characterScaling)
        {
            CharacterEnscaling(hit,
                               out float steadyEffect,
                               out float recklessEffect,
                               out float braveATKEffect,
                               out float braveSPDEffect,
                               out float calmReduceEffect,
                               out float currentOpATK,
                               out float currentMyRD,
                               out float currentMySPD);

            damage = (damage * currentOpATK / 100f)
            * ((100f - currentMyRD) / 100f);
        }

        // STAB and attribute effectiveness
        damage *= (100f + hit.stab + hit.attributeEffectiveness) / 100f;

        // Critical
        const float dmgbonus = 0;
        int luckIndex = Random.Range(0, 100) + 1;
        if (luckIndex <= opControlsScript.myInfo.luckPoints)
            damage = damage * (opControlsScript.myInfo.critPoints + dmgbonus) / 100f;
    ...
    private void CharacterEnscaling(Hit hit,
                                    out float steadyEffect,
                                    out float recklessEffect,
                                    out float braveATKEffect,
                                    out float braveSPDEffect,
                                    ...
                                    out float currentMySPD)
    {
        currentOpATK = opControlsScript.myInfo.attackPoints;
        currentMySPD = opControlsScript.myInfo.speedPoints;
        currentMyRD = myInfo.reducedamagePoints;

        //Cứng rắn Gia tăng + 20% Sát thương gây ra cho đối thủ,
        //khi bản thân còn 1/3 HP
        steadyEffect = 1f;
        if (hit.autoAbility
            && Array.Exists(opControlsScript.myInfo.natures, nature => nature.natureType == CharacterNatureType.Steady)
            && opControlsScript.currentLifePoints <= opControlsScript.myInfo.healthPoints / 3f)
        {
            steadyEffect = 1.2f;
            currentOpATK *= steadyEffect;
        }

        //Bẽn lẽn Có 20% cơ hội tự động xóa 1 trạng thái Tiêu cực (Negatively)
        //khi gây sát thương cho mục tiêu.
        bool removeNegative = false;
        StatusResistanceInfo[] currentOpHitNatures = new StatusResistanceInfo[0];
        if (Array.Exists(opControlsScript.myInfo.natures, nature => nature.natureType == CharacterNatureType.Bashful))
        {
            removeNegative = Random.Range(0, 100) < 20;
            if (removeNegative && opControlsScript.myInfo.statusResistances.Length > 0)
            {
                int iRemove = Random.Range(0, opControlsScript.myInfo.statusResistances.Length - 1);
                currentOpHitNatures = UFE.RemoveAt<StatusResistanceInfo>(myInfo.statusResistances, iRemove);
            }
        }

        //Táo bạo, mạnh dạn Gia tăng + 40% sát thương gây ra cho đối thủ,
        //khi bị dính trạng thái tiêu cực (Negatively)
        recklessEffect = 1f;

        if (Array.Exists(opControlsScript.myInfo.natures, nature => nature.natureType == CharacterNatureType.Reckless)
            && Array.Exists(opControlsScript.myInfo.currentImpact, negative => negative.resistanceGroup == StatusResistanceGroup.Negatively))
        {
            recklessEffect = 1.4f;
            currentOpATK *= recklessEffect;
        }

        //Can đảm Gia tăng 10 % ATK giảm 20 % tốc độ
        braveATKEffect = 1f;
        braveSPDEffect = 1f;

        if (Array.Exists(opControlsScript.myInfo.natures, nature => nature.natureType == CharacterNatureType.Brave))
        {
            braveATKEffect = 1.1f; currentOpATK *= braveATKEffect;
            braveSPDEffect = 0.8f; currentMySPD *= braveSPDEffect;
        }

        //Điềm tĩnh Gia tăng 3 % Giảm Sát thương(Reduce Damage)
        calmReduceEffect = 1f;

        if (Array.Exists(opControlsScript.myInfo.natures, nature => nature.natureType == CharacterNatureType.Calm))
        {
            calmReduceEffect = 1.03f;
        }

        //Cẩn thận Tăng 5 % (Giảm sát thương)  và làm giảm 5 % ATK
        //Ngoan ngoãn Reduce Damage(Giảm sát thương) không thể bị ảnh hưởng.
        //Dịu dàng    30 % cơ hội phục hồi một nửa HP khi còn 1 / 10 HP cơ bản
        //Lì lợm Khi hết HP, tự động giữ lại 1 HP trong 20s.Mọi sát thương không có tác dụng trong thời gian này
        //Lanh lẹ Gia tăng + 20 SPD khi vào trận đấu
        //Xấu tính Trạng bị + 5 EPS khi bắt đầu trận đấu, giảm 20 ATK khi bắt đầu trận đấu
        //Vui vẻ  Tăng SPD +10 , ATK + 10
        //Yếu đuối    Giảm 5 % (Giảm sát thương) tăng + 5 Luck
        //Cô đơn EPS tự động tăng 1 EPS / 2s
        //Nhạt    Gia tăng +50 % Crit damage nhưng giảm 3 % chỉ số Reduce Damage
        //Khiêm tốn Tất cả chỉ số không thể bị giảm
        //Ngây thơ Tăng +10 Luck và +50 % Crit Dmg khi bắt đầu trận đấu. Có hiệu quả trong 30s
        //Nghịch ngợm Tăng 10 % khả năng khiến đối thủ bị dính trạng thái bất lợi nếu đòn tấn công gây sát thương.
        //Yên lặng    ATK không thể bị giảm
        //Kỳ quặc Tăng 50 % Sát thương nếu đòn tấn công là Chí mạng .
        //Dại dột Gia tăng +30 ATK , giảm 5 % (Giảm sát thương) khi bắt đầu trận đấu.Có hiệu quả trong 30s.
        //Thư thái, Nho nhã   30 % cơ hội phục hồi 1 / 2 HP khi nhân vật còn 1 / 3 HP.
        //Hỗn láo Tăng 5 % Giảm sát thương  và làm giảm 20 SPD
        // Nghiêm túc SPD không thể bị giảm
        //Nhút nhát   Xác xuất 7 % né tránh các đòn tấn công của đối thủ
    }
    ...
    public bool IsStatusRunning<T>()
    {
        T status = GetComponent<T>();
        if (status == null) return false;
        iStatus istatus = status as iStatus;
        return istatus.IsRunning();
    }
    ...
    private void PausePlayAnimation(bool pause, Fix64 animSpeed)
    {
        ...
        if (pause)
        {
            myMoveSetScript.SetAnimationSpeed(myMoveSetScript.GetAnimationSpeed() * animSpeed);// * (myInfo.speedPoints / 100));
        }
        else
        {
    ...
    public void AddGauge(Fix64 gaugeGain, int targetGauge)
    {
        ...
        currentGaugesPoints[targetGauge] += (myInfo.maxEPS * (gaugeGain / 100));
        if (currentGaugesPoints[targetGauge] > myInfo.maxEPS) currentGaugesPoints[targetGauge] = myInfo.maxEPS;
    }
    ...
    private void RemoveGauge(Fix64 gaugeLoss, int targetGauge)
    {
        ...
        currentGaugesPoints[targetGauge] -= (myInfo.maxEPS * (gaugeLoss / 100));
    ...
    {
        currentLifePoints = myInfo.healthPoints;
        UFE.FireLifePoints(myInfo.healthPoints, opControlsScript);
    }

    private void RefillGauge()
    {
        for (int i = 0; i < currentGaugesPoints.Length; i++) AddGauge(myInfo.maxEPS, i);
    }
    ...
    private bool DamageMe(Fix64 damage)
    {
        ...
        if ((UFE.gameMode == GameMode.TrainingRoom || UFE.gameMode == GameMode.ChallengeMode)
            ...
            || (playerNum == 2 && UFE.config.trainingModeOptions.p2Life == LifeBarTrainingMode.Refill)))
        {
            if (currentLifePoints == 0) currentLifePoints = myInfo.healthPoints;
    ...
    public void ResetData(bool resetLife)
    {
        ...
        if (resetLife || UFE.config.roundOptions.resetLifePoints)
        {
            ...
            {
                currentLifePoints = (Fix64)myInfo.healthPoints * (UFE.config.trainingModeOptions.p1StartingLife / 100);
            }
            else if (playerNum == 2 && (UFE.gameMode == GameMode.TrainingRoom || UFE.gameMode == GameMode.ChallengeMode))
            {
                currentLifePoints = (Fix64)myInfo.healthPoints * (UFE.config.trainingModeOptions.p2StartingLife / 100);
            }
            else
            {
                currentLifePoints = myInfo.healthPoints;
