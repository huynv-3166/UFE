...
public class MoveSetScript : MonoBehaviour
{
        ...
        foreach (MoveSetData moveSetData in controlsScript.loadedMoves)
        {
            if (moveSetData.combatStance == newStance)
            {
                ...
                if (moveSetData.cinematicIntro != null)
                {
                    ...
                    intro.name = "Intro";
                    attachAnimation(intro.animMap.clip,
                                    intro.name,
                                    intro._animationSpeed,
                                    //controlsScript.myInfo.speedPoints,
                                    intro.wrapMode,
                ...
                if (moveSetData.cinematicOutro != null)
                {
                    ...
                    outro.name = "Outro";
                    attachAnimation(outro.animMap.clip,
                                    outro.name,
                                    outro._animationSpeed,
                                    //controlsScript.myInfo.speedPoints,
                                    outro.wrapMode,
    ...
    private void fillMoves()
    {
        ...
        foreach (MoveInfo move in moves)
        {
            ...
            {
                attachAnimation(move.animMap.clip,
                                move.name,
    ...
    private void setBasicMoveAnimation(BasicMoveInfo basicMove, string animName, BasicMoveReference basicMoveReference)
    {
        ...
        attachAnimation(basicMove.animMap[0].clip,
                        animName,
                        basicMove._animationSpeed,
                        //controlsScript.myInfo.speedPoints,
                        basicMove.wrapMode,
    ...
    private void attachAnimation(AnimationClip clip,
                                 string animName,
                                 Fix64 speed,
                                 //Fix64 speedCharacter,
                                 WrapMode wrapMode,
                                 Fix64 length)
    {
        ...
        if (controlsScript.myInfo.animationType == AnimationType.Legacy)
        {
            legacyControl.AddClip(clip, animName, speed
                                                        //* (speedCharacter / 100)
                                                        , wrapMode, length);
        }
        else
        {
            mecanimControl.AddClip(clip, animName, speed
                //* (speedCharacter / 100)
                , wrapMode, length);
        }
    }
    ...
    public void SetAnimationSpeed(string animationName,
                                  Fix64 speed
                                  //Fix64 characterSpeed
                                  )
    {
        if (controlsScript.myInfo.animationType == AnimationType.Legacy)
        {
            legacyControl.SetSpeed(animationName, speed);// * (characterSpeed / 100));
        }
        else
        {
            mecanimControl.SetSpeed(animationName, speed);// * (characterSpeed / 100));
        }
    }
    ...
    public bool CompareBlockButtons(ButtonPress button)
    {
        ...
        if (button == ButtonPress.LeftPress && UFE.config.blockOptions.blockType == BlockType.LeftPress) return true;
        return false;
    }
    ...
    private bool hasEnoughGauge(Fix64 gaugeNeeded, int targetGauge)
    {
        ...
        if (controlsScript.currentGaugesPoints[targetGauge] < ((Fix64)controlsScript.myInfo.maxEPS * (gaugeNeeded / 100))) return false;
        return true;
    }
    ...
    public bool ValidateMoveStances(PlayerConditions conditions, ControlsScript cScript, bool bypassCrouchStance)
    {
        ...
        foreach (PossibleMoveStates possibleMoveState in conditions.possibleMoveStates)
        {
            ...
            if (possibleMoveState.possibleSide == PossibleSide.Left && cScript.mirror > 0) continue;
            if (possibleMoveState.possibleSide == PossibleSide.Right && cScript.mirror < 0) continue;
