...
public class MoveEditorWindow : EditorWindow
{
    ...
    private bool attributesOptions;
    private bool statusResistanceOption;
    ...
    void Clear(bool destroyChar, bool resetCam, bool p1, bool p2, bool projectile)
    {
        ...
        if (resetCam)
        {
            ...
            Camera playerCam = sceneCam.transform.Find("Camera-Player").GetComponent<Camera>();
            if (sceneCam != null)
            {
                sceneCam.fieldOfView = initialFieldOfView;
                playerCam.fieldOfView = sceneCam.fieldOfView;
    ...
    public void OnGUI()
    {
        ...
        scrollPos = EditorGUILayout.BeginScrollView(scrollPos);
        {
            EditorGUILayout.BeginVertical(rootGroupStyle);
            {
                ...
                if (generalOptions)
                {
                    EditorGUILayout.BeginVertical(subGroupStyle);
                    {
                        ...
                        EditorGUILayout.BeginHorizontal();
                        {
                            moveInfo.skillBorderPicture = (Sprite)EditorGUILayout.ObjectField(moveInfo.skillBorderPicture, typeof(Sprite), false, GUILayout.Width(80), GUILayout.Height(60));
                            moveInfo.skillPicture = (Sprite)EditorGUILayout.ObjectField(moveInfo.skillPicture, typeof(Sprite), false, GUILayout.Width(80), GUILayout.Height(60));
                        }
                        EditorGUILayout.EndHorizontal();

                        moveInfo.skillsizeType = (SkillSizeType)EditorGUILayout.EnumPopup("Skill Type:", moveInfo.skillsizeType, enumStyle);
                        ...
                        EditorGUILayout.BeginVertical();
                        {
                            ...
                            moveInfo.characterScaling = EditorGUILayout.Toggle("Character Scaling", moveInfo.characterScaling, toggleStyle);
                            if (moveInfo.characterScaling)
                                moveInfo.autoAbility = EditorGUILayout.Toggle("Auto Ability", moveInfo.autoAbility, toggleStyle);
            ...
            EditorGUILayout.BeginVertical(rootGroupStyle);
            {
                EditorGUILayout.BeginHorizontal();
                {
                    attributesOptions = EditorGUILayout.Foldout(attributesOptions, "Attributes", foldStyle);
                    helpButton("attributes:prefabs");
                }
                EditorGUILayout.EndHorizontal();
                ...
                if (attributesOptions)
                {
                    EditorGUILayout.BeginVertical(subGroupStyle);
                    {
                        ...
                        for (int i = 0; i < moveInfo.moveAtrributes.Length; i++)
                        {
                            EditorGUILayout.Space();
                            EditorGUILayout.BeginVertical(arrayElementStyle);
                            {
                                ...
                                EditorGUILayout.BeginHorizontal();
                                {
                                    moveInfo.moveAtrributes[i] = (CharacterAttributeInfo)EditorGUILayout.ObjectField("Attribute:", moveInfo.moveAtrributes[i], typeof(CharacterAttributeInfo), true);
                                    if (GUILayout.Button("", "PaneOptions"))
                                    {
                                        PaneOptions<CharacterAttributeInfo>(moveInfo.moveAtrributes, moveInfo.moveAtrributes[i], delegate (CharacterAttributeInfo[] newElement) { moveInfo.moveAtrributes = newElement; });
                                    }
                                }
                                EditorGUILayout.EndHorizontal();
                            }
                        ...
                        if (StyledButton("New Effectiveness Link"))
                            moveInfo.moveAtrributes = AddElement<CharacterAttributeInfo>(moveInfo.moveAtrributes, null);
            ...
            EditorGUILayout.BeginVertical(rootGroupStyle);
            {
                EditorGUILayout.BeginHorizontal();
                {
                    statusResistanceOption = EditorGUILayout.Foldout(statusResistanceOption, "Status Resistances", foldStyle);
                    helpButton("rareness:prefabs");
                }
                EditorGUILayout.EndHorizontal();


                if (statusResistanceOption)
                {
                    EditorGUILayout.BeginVertical(subGroupStyle);
                    {
                        EditorGUI.indentLevel += 1;

                        for (int i = 0; i < moveInfo.statusResistances.Length; i++)
                        {
                            EditorGUILayout.Space();
                            EditorGUILayout.BeginVertical(arrayElementStyle);
                            {
                                ...
                                EditorGUILayout.BeginHorizontal();
                                {
                                    moveInfo.statusResistances[i] = (StatusResistanceInfo)EditorGUILayout.ObjectField("Status Resistance:", moveInfo.statusResistances[i], typeof(StatusResistanceInfo), true);
                                    if (GUILayout.Button("", "PaneOptions"))
                                    {
                                        PaneOptions<StatusResistanceInfo>(moveInfo.statusResistances, moveInfo.statusResistances[i], delegate (StatusResistanceInfo[] newElement) { moveInfo.statusResistances = newElement; });
                                    }
                                }
                                EditorGUILayout.EndHorizontal();
                            }
                        ...
                        if (StyledButton("New Status Resistance Link"))
                            moveInfo.statusResistances = AddElement<StatusResistanceInfo>(moveInfo.statusResistances, null);
                    ...
                    EditorGUILayout.BeginVertical(subGroupStyle);
                    {
                        ...
                        if (moveInfo.animMap.clip != null)
                        {
                            ...
                            if (moveInfo.animMap.hitBoxDefinitionType == HitBoxDefinitionType.AutoMap)
                            {
                                ...
                                //moveInfo.scaleCharacterSpeed = EditorGUILayout.Toggle("Scale with Character Speed", moveInfo.scaleCharacterSpeed, toggleStyle);
                                    ...
                                    EditorGUILayout.BeginVertical(subGroupStyle);
                                    {
                                        ...
                                        for (int k = 0; k < moveInfo.opponentOverride[i].characterSpecificMoves.Length; k++)
                                        {
                                            ...
                                            EditorGUILayout.BeginVertical(subArrayElementStyle);
                                            {
                                                ...
                                                PlayerConditionsGroup("Advanced Filters", moveInfo.opponentOverride[i].characterSpecificMoves[k].conditions, false);
                                                EditorGUILayout.Space();
                                            }
                                            EditorGUILayout.EndVertical();
                                        }
                    ...
                    EditorGUILayout.BeginVertical(subGroupStyle);
                    {
                        ...
                        for (int i = 0; i < moveInfo.characterAssist.Length; i++)
                        {
                            ...
                            EditorGUILayout.BeginVertical(arrayElementStyle);
                            {
                                ...
                                moveInfo.characterAssist[i].characterInfo = (TOHDragonFight3D.CharacterInfo)EditorGUILayout.ObjectField("Character:", moveInfo.characterAssist[i].characterInfo, typeof(TOHDragonFight3D.CharacterInfo), true);
            ...
            EditorGUILayout.BeginVertical(rootGroupStyle);
            {
                ...
                if (particleEffectsOptions)
                {
                    EditorGUILayout.BeginVertical(subGroupStyle);
                    {
                        EditorGUI.indentLevel += 1;
                        List<int> castingValues = new List<int>();
                        foreach (AbstractParticleEffect particleEffect in moveInfo.particleEffects)
                        ...
                        for (int i = 0; i < moveInfo.particleEffects.Length; i++)
                        {
                            ...
                            EditorGUILayout.BeginVertical(arrayElementStyle);
                            {
                                ...
                                EditorGUILayout.BeginHorizontal();
                                {
                                    ...
                                    if (GUILayout.Button("", "PaneOptions"))
                                    {
                                        PaneOptions<AbstractParticleEffect>(moveInfo.particleEffects, moveInfo.particleEffects[i], delegate (AbstractParticleEffect[] newElement) { moveInfo.particleEffects = newElement; });
                                    }
                                }
                                EditorGUILayout.EndHorizontal();
                                ...
                                moveInfo.particleEffects[i].particleEffect.identity = (IdentityType)EditorGUILayout.EnumPopup("Identity:", moveInfo.particleEffects[i].particleEffect.identity, enumStyle);
                        ...
                        if (StyledButton("New Particle Effect"))
                            moveInfo.particleEffects = AddElement<AbstractParticleEffect>(moveInfo.particleEffects, new AbstractParticleEffect());
                    ...
                    EditorGUILayout.BeginVertical(subGroupStyle);
                    {
                        ...
                        for (int i = 0; i < moveInfo.appliedForces.Length; i++)
                        {
                            ...
                            EditorGUILayout.BeginVertical(arrayElementStyle);
                            {
                                ...
                                moveInfo.appliedForces[i].towardOpponent = EditorGUILayout.Toggle("Toward Opponent", moveInfo.appliedForces[i].towardOpponent, toggleStyle);
                                moveInfo.appliedForces[i].backwardOpponent = EditorGUILayout.Toggle("Backward Opponent", moveInfo.appliedForces[i].backwardOpponent, toggleStyle);
                                if (moveInfo.appliedForces[i].towardOpponent == true
                                    && moveInfo.appliedForces[i].backwardOpponent == true)
                                {
                                    moveInfo.appliedForces[i].towardOpponent = false;
                                    moveInfo.appliedForces[i].backwardOpponent = false;
                                }
                                if (moveInfo.gameplayType == GameplayType._2DFighter)
                                {
                                ...
                                EditorGUI.BeginDisabledGroup(moveInfo.projectiles[i].projectilePrefab == null);
                                {
                                    ...
                                    if (moveInfo.projectiles[i].damageOptionsToggle)
                                    {
                                        EditorGUILayout.BeginVertical(subGroupStyle);
                                        {
                                            ...
                                            moveInfo.projectiles[i].characterScaling = EditorGUILayout.Toggle("Character Scaling", moveInfo.projectiles[i].characterScaling, toggleStyle);
                                    ...
                                    {
                                        EditorGUI.indentLevel += 1;
                                        //moveInfo.projectiles[i].initMove = (MoveInfo)EditorGUILayout.ObjectField("Causing Move:", moveInfo.projectiles[i].initMove, typeof(MoveInfo), true);

                                        for (int projectileIndex = 0;
                                            projectileIndex < moveInfo.projectiles[i].projectileAtrributes.Length;
                                            projectileIndex++)
                                        {
                                            EditorGUILayout.Space();
                                            EditorGUILayout.BeginVertical(arrayElementStyle);
                                            {
                                                ...
                                                EditorGUILayout.BeginHorizontal();
                                                {
                                                    moveInfo.projectiles[i].projectileAtrributes[projectileIndex] = (CharacterAttributeInfo)EditorGUILayout.ObjectField("Attribute:", moveInfo.projectiles[i].projectileAtrributes[projectileIndex], typeof(CharacterAttributeInfo), true);
                                                    if (GUILayout.Button("", "PaneOptions"))
                                                    {
                                                        PaneOptions<CharacterAttributeInfo>(moveInfo.projectiles[i].projectileAtrributes, moveInfo.projectiles[i].projectileAtrributes[projectileIndex], delegate (CharacterAttributeInfo[] newElement) { moveInfo.projectiles[i].projectileAtrributes = newElement; });
                                                    }
                                                }
                                                EditorGUILayout.EndHorizontal();
                                            }
                                        ...
                                        if (StyledButton("New Attribute Link"))
                                            moveInfo.projectiles[i].projectileAtrributes = AddElement<CharacterAttributeInfo>(moveInfo.projectiles[i].projectileAtrributes, null);
            ...
            EditorGUILayout.BeginVertical(subGroupStyle);
            {
                ...
                for (int i = 0; i < playerConditions.possibleMoveStates.Length; i++)
                {
                    ...
                    EditorGUILayout.BeginVertical(subArrayElementStyle);
                    {
                        ...
                        playerConditions.possibleMoveStates[i].possibleSide = (PossibleSide)EditorGUILayout.EnumPopup("Side:", playerConditions.possibleMoveStates[i].possibleSide, enumStyle);
                        ...
                        float pArcBeginsTemp = playerConditions.possibleMoveStates[i].proximityRangeBegins;
                        float pArcEndsTemp = playerConditions.possibleMoveStates[i].proximityRangeEnds;
                        EditorGUI.indentLevel += 2;
    ...
    public void StyledMinMaxSlider(string label, ref float minValue, ref float maxValue, int minLimit, int maxLimit, int indentLevel)
    {
        //      int indentSpacing = 25 * indentLevel;
        //      //indentSpacing += 30;
        //      EditorGUILayout.Space();
        ...
        //      //centeredStyle.alignment = TextAnchor.UpperCenter;
        //      labelStyle.alignment = TextAnchor.UpperCenter;
        //      GUI.Label(rect, label + " between " + minValueFloat + " and " + maxValueFloat, labelStyle);
        //      labelStyle.alignment = TextAnchor.MiddleCenter;
        ...
        //      EditorGUI.MinMaxSlider(rect, ref minValueFloat, ref maxValueFloat, minLimitFloat, maxLimitFloat);
        //      minValue = (float)Mathf.Floor(minValueFloat);
        //      maxValue = (float)Mathf.Floor(maxValueFloat);

        //      tempRect = GUILayoutUtility.GetRect(1, 20);

        minValue = EditorGUILayout.FloatField("Min ", (float)minValue);
        maxValue = EditorGUILayout.FloatField("Max ", (float)maxValue);
    }
