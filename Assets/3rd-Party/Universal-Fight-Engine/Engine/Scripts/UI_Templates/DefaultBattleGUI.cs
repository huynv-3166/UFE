...
namespace TOHDragonFight3D
{
    public class DefaultBattleGUI : BattleGUI
    {
        ...
        public class PlayerGUI
        {
            ...
            public Image[] gauges;
            public Text[] gaugesCounts;
            [HideInInspector] public Fix64[] gaugesCost;
            ...
            if (this.isRunning)
            {
                ...
                if (UFE.config.gameGUI.hasGauge)
                {
                    for (int i = 0; i < this.player1GUI.gauges.Length; i++)
                    {
                        ...
                        {
                            int count = (float)this.player1GUI.gaugesCost[i] != 0 ? (int)((float)player1.controlsScript.currentGaugesPoints[i] / (float)this.player1GUI.gaugesCost[i]) : int.MaxValue;
                            this.player1GUI.gaugesCounts[i].text = count != int.MaxValue ? count.ToString() : "oo";
                            this.player1GUI.gauges[i].fillAmount = count != int.MaxValue ?
                                (count == 0 ? ((float)player1.controlsScript.currentGaugesPoints[i] - count * (float)this.player1GUI.gaugesCost[i]) / (float)this.player1GUI.gaugesCost[i] : 1)
                                : 1;
                        }
                    }
                    for (int i = 0; i < this.player2GUI.gauges.Length; i++)
                    {
                        ...
                        {
                            int count = (float)this.player2GUI.gaugesCost[i] != 0 ? (int)((float)player2.controlsScript.currentGaugesPoints[i] / (float)this.player2GUI.gaugesCost[i]) : int.MaxValue;
                            this.player2GUI.gaugesCounts[i].text = count != int.MaxValue ? count.ToString() : "oo";
                            this.player2GUI.gauges[i].fillAmount = count != int.MaxValue ?
                                (count == 0 ? ((float)player2.controlsScript.currentGaugesPoints[i] - count * (float)this.player2GUI.gaugesCost[i]) / (float)this.player2GUI.gaugesCost[i] : 1)
                                : 1;
                        }
                    }
                }
        ...
        {
            TOHDragonFight3D.CharacterInfo character = controlsScript != null ? controlsScript.myInfo : null;
        ...
        protected override void OnGameBegin(ControlsScript cPlayer1, ControlsScript cPlayer2, StageOptions stage)
        {
            ...
            FillInAttributes(cPlayer1.myInfo, player1GUI.attributes);
            FillInAttributes(cPlayer2.myInfo, player2GUI.attributes);

            // Set stars rareness
            FillInRareness(cPlayer1.myInfo.rareness, player1GUI.rareness);
            FillInRareness(cPlayer2.myInfo.rareness, player2GUI.rareness);
            ...
            if (UFE.config.gameGUI.hasGauge)
            {
                for (int i = 0; i < this.player1GUI.gauges.Length; i++)
                {
                    ...
                    this.player1GUI.gauges[i].fillAmount = (float)cPlayer1.currentGaugesPoints[i] / UFE.config.player1Character.maxEPS;
                }
                ...
                for (int i = 0; i < this.player2GUI.gauges.Length; i++)
                {
                    ...
                    this.player2GUI.gauges[i].fillAmount = (float)cPlayer2.currentGaugesPoints[i] / UFE.config.player2Character.maxEPS;
                }
            }
        }
        ...
        protected override void OnRoundEnd(ControlsScript winner, ControlsScript loser)
        {
            ...
            if (this.announcer != null && !this.muteAnnouncer)
            {
                ...
                if (winner.currentLifePoints == winner.myInfo.healthPoints)
                {
        ...
        protected override void FillInAttributes(TOHDragonFight3D.CharacterInfo characterInfo, Image[] attributeImages)
        {
            base.FillInAttributes(characterInfo, attributeImages);

            //Set the Character attributes
            int maxLength = Math.Max(characterInfo.characterAtrributes.Length, attributeImages.Length);
            for (int i = 0; i < maxLength; i++)
            {
                if (i < characterInfo.characterAtrributes.Length &&
                    i < attributeImages.Length)
                {
                    attributeImages[i].sprite = characterInfo.characterAtrributes[i].attributePicture;
                    attributeImages[i].gameObject.SetActive(true);
                }
                else if (i >= characterInfo.characterAtrributes.Length
                    && i < attributeImages.Length)
                {
                    attributeImages[i].gameObject.SetActive(false);
                }
            }
        }

        protected override void FillInRareness(int rareness, Image[] rareImages)
        {
            base.FillInRareness(rareness, rareImages);

            //Set the Character attributes
            int maxLength = Math.Max(rareness, rareImages.Length);
            for (int i = 0; i < maxLength; i++)
            {
                if (i < rareness && i < rareImages.Length)
                {
                    rareImages[i].gameObject.SetActive(true);
                }
                else if (i >= rareness && i < rareImages.Length)
                {
                    rareImages[i].gameObject.SetActive(false);
                }
            }
        }

        [ContextMenu("DebugMyPlayer")]
        void DebugMyPlayer()
        {
            var player1 = UFE.GetPlayer1ControlsScript();
            Debug.Log(player1.mirror + " pos " + player1.transform.position);
            var player2 = UFE.GetPlayer2ControlsScript();
            Debug.Log(player2.mirror + " pos " + player2.transform.position);
        }

        [ContextMenu("RefreshSkillButtons")]
        protected override void RefreshSkillButtons()
        {
            base.RefreshSkillButtons();
            switch (UFE.gameMode)
            {
                ...
                case GameMode.ChallengeMode:
                    FillInSkillPlayer(UFE.localPlayerController.player);
                    break;
            }
        }

        void FillInSkillPlayer(int player)
        {
            var skillButtonParent = Array.Find(UFE.controlFreakPrefab.transform.GetComponentsInChildren<Transform>(),
                parrent => parrent.name.Equals("Player-Skill-Buttons"));
            PlayerGUI playerGUI = player == 1 ? player1GUI : player2GUI;
            TOHDragonFight3D.CharacterInfo characterInfo = player == 1 ? UFE.GetPlayer1() : UFE.GetPlayer2();

            if (skillButtonParent != null)
            {
                var allChilds = skillButtonParent.GetComponentsInChildren<Transform>();
                var allSkill = Array.FindAll(allChilds,
                    skillButton => skillButton.name.Contains("Skill")
                    && skillButton.name.Contains("Button")
                    && skillButton.name.Contains("Root"));

                List<TouchButtonSpriteAnimator> list_BG = new List<TouchButtonSpriteAnimator>();
                List<Image> list_Gauge = new List<Image>();
                List<Text> list_GaugeCount = new List<Text>();
                List<Fix64> list_GaugeCost = new List<Fix64>();

                for (int i = 0; i < allSkill.Length; i++)
                {
                    TouchButtonSpriteAnimator gauge_bg = allSkill[i].childCount > 0 ? allSkill[i].GetChild(0).GetComponent<TouchButtonSpriteAnimator>() : null;
                    if (gauge_bg == null) continue;
                    list_BG.Add(gauge_bg);
                }

                if (characterInfo.stanceResourcePath.Length == 0) return;
                MoveSetData DF3Moves = Resources.Load<StanceInfo>(characterInfo.stanceResourcePath[0]).ConvertData();
                if (DF3Moves == null) return;

                int maxLoop = Math.Max(DF3Moves.skillMoves.Length, list_BG.Count);
                Vector2 currentLeftBot = new Vector2(0, 0); //First bot-left position is (30, 30)
                int pacing = 10;
                for (int i = 0; i < maxLoop; i++)
                {
                    Transform trsBG = list_BG[i].transform;
                    if (i >= DF3Moves.skillMoves.Length)
                        trsBG.parent.gameObject.SetActive(false);
                    else
                    {
                        Image gauge = trsBG.childCount > 0 ? trsBG.GetChild(0).GetComponent<Image>() : null;
                        if (gauge == null) continue;

                        //Handle button by on/off first and then set pos, set sprite later
                        trsBG.parent.gameObject.SetActive(false);
                        list_BG[i].SetSprite(DF3Moves.skillMoves[i].skillBorderPicture);
                        Vector2 buttonSize = DF3Moves.skillMoves[i].skillsizeType == SkillSizeType.Special ? new Vector2(100f, 100f) : new Vector2(75f, 75f);
                        SetSkillButonPlacement(trsBG.parent.GetComponent<TouchButton>(),
                            list_BG[i],
                            buttonSize,
                            currentLeftBot);
                        currentLeftBot += new Vector2(buttonSize.x + pacing, 0);
                        trsBG.parent.gameObject.SetActive(true);

                        //Handle Sprite
                        gauge.sprite = DF3Moves.skillMoves[i].skillPicture;
                        list_Gauge.Add(gauge);

                        //Handle Count
                        Text gaugeCount = gauge.GetComponentInChildren<Text>();
                        if (gaugeCount != null)
                        {
                            list_GaugeCount.Add(gaugeCount);
                            var gaugeCost = DF3Moves.skillMoves[i].gauges.Length > 0 ? DF3Moves.skillMoves[i].gauges[0] : null;
                            if (gaugeCost != null)
                                list_GaugeCost.Add(gaugeCost._gaugeRequired);
                        }
                    }
                }

                playerGUI.gauges = list_Gauge.ToArray();
                playerGUI.gaugesCounts = list_GaugeCount.ToArray();
                playerGUI.gaugesCost = list_GaugeCost.ToArray();
            }
        }

        void SetSkillButonPlacement(
            TouchButton rootButton,
            TouchButtonSpriteAnimator buttonSprite,
            Vector2 size,
            Vector2 leftbotPos)
        {
            RectTransform rootRect = rootButton.GetComponent<RectTransform>();
            rootRect.localPosition = leftbotPos;
            rootRect.sizeDelta = size;
            buttonSprite.Center();
            rootButton.ForceInit();
        }
        #endregion
        /*
        // DEBUG INFORMATION
