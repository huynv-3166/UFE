...
public class CharacterSelectionScreen : UFEScreen
{
    ...
    protected int p1HoverSkinIndex = 0;
    protected int p2HoverSkinIndex = 0;

    protected bool closing = false;
    protected TOHDragonFight3D.CharacterInfo[] selectableCharacters = new TOHDragonFight3D.CharacterInfo[0];
    #endregion
    ...
    public virtual void OnCharacterSelectionAllowed(int skinIndex, int player)
    {
        // If we haven't started loading a different screen....
        if (!this.closing)
        {
            // Check if we are trying to select or deselect a character...
            if (skinIndex >= 0 && skinIndex <= this.GetMaxCharacterIndex())
            {
                ...
                )
                {
                    ...
                    this.SetHoverIndex(player, skinIndex);
                    TOHDragonFight3D.CharacterInfo character = this.selectableCharacters[skinIndex];
    ...
    public void TrySelectCharacter(int skinIndex)
    {
        if (!UFE.isConnected)
        {
            // If it's a local game, update the corresponding character immediately...
        ...
        else
        {
            // If it's an online game, find out if the local player is Player1 or Player2
            // and update the selection only for the local player...
            this.TrySelectCharacter(skinIndex, UFE.GetLocalPlayer());
        }
    }

    public virtual void TrySelectCharacter(int skinIndex, int player)
    {
        // Check if he was playing online or not...
        if (!UFE.isConnected)
        {
            // If it's a local game, update the corresponding character immediately...
            this.OnCharacterSelectionAllowed(skinIndex, player);
        }
        else
        {
            ...
            if (player == localPlayer)
            {
                ...
                UFE.fluxCapacitor.RequestOptionSelection(localPlayer, (sbyte)skinIndex);
            }
        }
    }
    #endregion
